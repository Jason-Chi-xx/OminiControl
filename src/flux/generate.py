...
def generate(
    ...
    **params: dict,
    ...
    with torch.no_grad():
        text_query = model.mllm(
            image=image_base64,
            prompt=prompt,
        )
        image_pils = model.clip_processor(images=image_pils, return_tensors="pt").to(device)
        pooled_image_embeds = model.image_encoder(**image_pils).pooler_output
        pooled_prompt_embeds = model.clip_vision_projection(pooled_image_embeds)
    # condition_query = model.Query(text_query)
    prompt_embeds = model.connector(text_query)
    (
        prompt_embeds,
        pooled_prompt_embeds,
        text_ids,
